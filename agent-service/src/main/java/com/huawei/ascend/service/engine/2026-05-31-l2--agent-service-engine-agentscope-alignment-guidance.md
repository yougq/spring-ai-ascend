...
| `LocalDeployManager`（起 Spring Boot） | `web/.../LocalDeployManager.java` | `PlatformApplication`（已有单进程）+ `EngineCommandSubscriber`（队列消费） |
...
| 部署 | `AgentApp.run(port)` 独立端口 | 随 `PlatformApplication` 单进程，`EngineCommandSubscriber` auto-start |
...
| 启动代码放哪 | `PlatformApplication`（唯一 `@SpringBootApplication`）；`EngineCommandSubscriber` 随进程 auto-start 消费队列触发执行 |
