...
- 本地 LLM 选择远端 tool 后，只调用 runtime 中性服务 `RemoteAgentInvocationService`，不直接接触 A2A client。
...
A2aRemoteAgentCardFetcher
  ...
  -> BaseAgent AbilityManager + Runner.resourceMgr()
```

v1 采用“一远端 runtime 一个 tool”，不把远端多个 skill 拆成多个本地 tool。这样先保证状态与 resume 闭环稳定，再考虑细粒度 skill 暴露。
...
| OpenJiuwen tool | 当前 tool 调用线程等待 terminal 或 input-required | 不等待远端完成，进入 waiting/interrupted |
...
3. `OpenJiuwenRemoteToolAdapter` / installer / handler factory。
4. `RemoteAgentInvocationService` + `RemoteInvocationControl`。
5. `A2aRemoteAgentOutboundAdapter.invoke(...)`。
6. 覆盖 terminal success、failure、timeout、input-required。

### Phase 3 — Background + Input Required Resume

目标：让远端长任务和多轮交互稳定运行。

1. `submit(...)`、remote task reference、watcher。
2. `markSubmitted(...)`、`markInputRequired(...)`。
...
- OpenJiuwen remote tool 只调用 `RemoteAgentInvocationService`，不直接调用 A2A client。
...
6. 是否接受目录树为第一轮目标切片，A2A 打穿后允许刷新物理目录？
