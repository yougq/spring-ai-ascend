...
OpenJiuwen 示例会在启动期通过 runtime 的 OpenJiuwen checkpointer configurer
设置原生 checkpointer。默认路径使用 `InMemoryCheckpointer`，便于本地 E2E；
如果需要切到 Redis 路径，可以设置 `SAA_SAMPLE_OPENJIUWEN_CHECKPOINTER=redis`，
并通过 `SAA_SAMPLE_OPENJIUWEN_REDIS_URL` 指定 Redis URL。
